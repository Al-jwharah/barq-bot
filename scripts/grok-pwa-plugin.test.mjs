...
/** Isolate OG tests from this product's public/og.jpg + src/lib/og/site.json (برق). */
const EMPTY_OG_CWD = mkdtempSync(join(tmpdir(), "barq-og-"));
function isolate(ctx = {}) {
  return { site: {}, cwd: EMPTY_OG_CWD, ...ctx };
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, isolate({ appName: "Wild Race" }));
...
test("published grok.me slug is still a title fallback", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", isolate({
    host: "wild-race.grok.me",
  }));
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = injectGrokPwaHead("<html><head></head></html>", isolate({
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  }));
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = injectGrokPwaHead("<html><head></head></html>", isolate({
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", isolate({ appName: "Solo" }));
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector(isolate({ appName: "Wild Race" }));
  const chunks = [
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", isolate({ appName: "Wild Race" }));
